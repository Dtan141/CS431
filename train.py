...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(opt.mask_datapath) > 0:
            config.model.params.attention_store_background_mask = True
        if opt.attention_store_step:
            config.model.params.attention_store_step = True
                ...
                'metrics_over_trainsteps_checkpoint':
                    {"target": 'pytorch_lightning.callbacks.ModelCheckpoint',
                     'params': {
                         ...
                         "verbose": True,
                         'save_top_k': -1,
                         'every_n_train_steps': 50, # origin 50
        ...
        # # 遍历属性名，找出所有私有参数
        # private_vars = [v for v in all_vars.keys()]

        # # 打印所有私有参数名和参数值
        # for v in private_vars:
        #     print(f"{v}: {all_vars[v]}")
